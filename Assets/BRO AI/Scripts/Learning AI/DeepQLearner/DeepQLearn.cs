...
namespace DeepQLearning
{
    ...
    public class DeepQLearn
    {
        ...
        public double AverageQLoss
        {
            get { return _avergageLossWindow.get_average(); }
        }

        public double SmoothishReward
        {
            get { return _averageRewardWindow.get_average(); }
        }
        #endregion
        ...
        public DeepQLearn(int numStates, int numActions, TrainingOptions opt)
        {
            ...
            if (opt.randomActionDistribution != null)
            {
                // this better sum to 1 by the way, and be of length numActions
                _randomActionDistribution = opt.randomActionDistribution;
        ...
        private Action Policy(double[] state, bool forward)
        {
            ...
            if (forward)
            {
                //PersistenceUtil.WriteCSVLine(Array.ConvertAll(actionValues.w, x => x.ToString())); // Logging the all action values
            }
